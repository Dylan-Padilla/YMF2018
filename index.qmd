...
Arrows(x0 = loc1.1$BP, y0 = 7, x1 = loc1.1$BP, y1 = 9, col = "red",
       arr.type = "triangle", code = 1, lwd = 1.5, arr.length = 0.2)
text(x = loc1.1$BP, y = 10, labels = "Rab28", cex = 0.6, col = "red")
...
lines(x, as.numeric(wide_df2[945, ]), col = "gold", lwd = 3)
lines(x, as.numeric(wide_df2[2247, ]), col = "skyblue", lwd = 3)
legend("topleft", legend = c("col16a1", "Rab28"), lty = 1, lwd = 2, bty = "n",
       col = c("skyblue", "gold"), cex = 0.8)
...
temp_pref <- read.table("/Users/dpadil10/ASU\ Dropbox/Dylan\ Padilla/fineScaleWoodFrog/fieldwork2021/temperaturePreference/tPrefSummary.tsv", header = TRUE, sep = "\t")
head(temp_pref)
str(temp_pref)

temp_pref2 <- read.csv("/Users/dpadil10/ASU\ Dropbox/Dylan\ Padilla/fineScaleWoodFrog/fieldwork2021/temperaturePreference/data/specimenDatabase_temperaturePreference.csv", header = TRUE)
head(temp_pref2)
str(temp_pref2)

merg.temp <- data.frame(ID = temp_pref2$ID, Clutch = temp_pref2$Clutch)
head(merg.temp)

## merging tables so I have information on clutch/family

temp.dat <- merge(temp_pref, merg.temp, by = "ID")
head(temp.dat)
str(temp.dat)


## setting up the CSV file with columns: population, family, Tpref)

Tpref <- temp.dat
names(Tpref)[6:7] <- c("Population", "Family")
str(Tpref)
Tpref$Family <- as.character(Tpref$Family)

## I have to remove clutches/families with less than 10 individuals.
## Otherwise, the model can't estimate the variances

Tpref <- Tpref[!Tpref$Population == "GB", ]
Tpref <- Tpref[!Tpref$Family == "3", ]
Tpref <- Tpref[!Tpref$Family == "7", ]
Tpref <- Tpref[!Tpref$Family == "16", ]
Tpref <- Tpref[!Tpref$Family == "19", ]

tapply(Tpref$mean, Tpref$Population, length)
tapply(Tpref$mean, Tpref$Family, length)

## fitting the Mixed-Effects Model
## we treat Population and Family (nested in Population) as random effects
## to estimate their variance components
Tpref.model <- lmer(mean ~ (1 | Population) + (1 | Population:Family), 
                      data = Tpref)
summary(Tpref.model)

## extracting Variance Components
Tpref_var.comp <- as.data.frame(VarCorr(Tpref.model))
print(Tpref_var.comp)

## variables for Qst calculation:
## sigma2_between (Population variance)
var_pop <- Tpref_var.comp$vcov[Tpref_var.comp$grp == "Population"]

## sigma2_within (Family variance - usually multiplied to
##estimate additive genetic variance Va)
## in a half-sib design, Va = 4 * var_family.
## In full-sib, it's roughly 2 * var_family.
var_fam <- Tpref_var.comp$vcov[Tpref_var.comp$grp == "Population:Family"]
va_est <- 2 * var_fam 

## calculating Qst
## formula: Qst = var_pop / (var_pop + 2 * va_est)
qst <- var_pop / (var_pop + 2 * va_est)
qst  ## EVEN THOUGH I REMOVED CLUTCHES WITH LOW NUMBER OF
## INDIVIDUALS, Qst ENDED UP BEING TOO LOW
wc84_stats$FST
...
qst_boot_func <- function(data, indices) {
  ...
  ## fit the model (using blmer to avoid singular fits)
  Tpref.model <- lmer(mean ~ (1 | Population) + (1 | Population:Family), data = d)
...
## running 1000 bootstrap iterations
##set.seed(123) # for reproducibility
boot_results <- boot(data = Tpref, statistic = qst_boot_func, R = 1000)
...
Tpref.tab <- data.frame(Trait = "Tpref", Qst = qst,
                        Qst_ci_ll = boot_ci_results$bca[4],
                        Qst_ci_ul = boot_ci_results$bca[5],
                        Fst = wc84_stats$FST,
                        Fst_ci_ll = fst_ll,
...
## we treat Population and Family (nested in Population) as random effects
## to estimate their variance components
EP.model <- lmer(EP ~ (1 | Population) + (1 | Population:Family), 
                      data = EP)
summary(EP.model)
...
qst_boot_func <- function(data, indices) {
  ...
  ## fit the model (using blmer to avoid singular fits)
  EP.model <- lmer(EP ~ (1 | Population) + (1 | Population:Family), data = d)
